...
import { generateNormals, generateTangents, gltfEnum, newTypedArray, toIndices, TypedArray } from "../utils/gltfUtils";
...
export class GLTF {
	...
	async parseData() {
		this.buffers = await this.loadBuffer();
	...
	private createGeometry(primitive, material) {
		...
		if (primitive.indices !== undefined) {
			...
			indices = toIndices(accessor.values);
			vertexCount = accessor.count;
		}
		let normals;
	...
	private parseNodeTRS(node: Node, gltfNode: GLTFNodeParams): Node {
